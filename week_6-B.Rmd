...
tibble(urn_red = seq(100, 700, by = 100)) %>% 
  # . is what you've mentioned from above, references urn_red
  #rbinom creates table: number of pulls 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 100, size = 25, prob = ./1000))) %>% 
  unnest(paddle_red) %>% 
  #select ten random ones
  sample_n(10)
```
...
ggplot(aes(paddle_red, urn_red)) +
  geom_point(alpha = .01) +
  labs(title = "Joint Distribution of Red Beads in Urn & Paddle",
